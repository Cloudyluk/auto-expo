  ...
  return <details className="language-menu">
    ...
    <div className="language-popover" aria-label="Language">
      {LANGUAGE_MENU.map((language) => language.status === 'available'
        ? <a key={language.code} data-language-name="true" href={language.code === 'en' ? '/' : `/${language.code}/`} aria-current={language.code === locale ? 'page' : undefined}><span aria-hidden="true">{language.flag}</span><span>{language.label}</span></a>
