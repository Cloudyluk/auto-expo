...
const countryNames = { en: { CN: 'China', US: 'United States', DE: 'Germany', JP: 'Japan', KR: 'South Korea', TH: 'Thailand', IN: 'India', BR: 'Brazil', GB: 'United Kingdom', FR: 'France', IT: 'Italy', CA: 'Canada', MX: 'Mexico', AE: 'United Arab Emirates', INTL: 'International' }, es: { CN: 'China', US: 'Estados Unidos', DE: 'Alemania', JP: 'Japón', KR: 'Corea del Sur', TH: 'Tailandia', IN: 'India', BR: 'Brasil', GB: 'Reino Unido', FR: 'Francia', IT: 'Italia', CA: 'Canadá', MX: 'México', AE: 'Emiratos Árabes Unidos', INTL: 'Internacional' } };
...
export function App({ pathname = window.location.pathname }) {
  ...
  return (
    <main data-locale={locale}>
      ...
      {page === 'map' && <section><h1>{t(locale, 'map.title')}</h1><p>{t(locale, 'map.distribution')}</p><div className="country-map">{countries.map((code) => <button key={code} onClick={() => { setCountry(code); setPage('country'); }}>{(countryNames[locale] || countryNames.en)[code]} <b>{EXHIBITIONS.filter((event) => countryFor(event) === code).length}</b></button>)}</div></section>}
