...
export function App({ pathname = window.location.pathname }) {
  const locale = pathname.startsWith('/zh') ? 'zh' : pathname.startsWith('/es') ? 'es' : pathname.startsWith('/pt') ? 'pt' : pathname.startsWith('/fr') ? 'fr' : pathname.startsWith('/de') ? 'de' : 'en';
  ...
  return (
    <main data-locale={locale}>
      ...
      {page === 'sales' && <section><h1>{t(locale, 'sales.title')}</h1><div className="path-grid">{Object.entries(SALES_ROUTES).map(([routeId, route]) => <section key={routeId}><h2>{pathLabel({ id: routeId, label: route.label }, locale)}</h2>{getSalesRouteProducts(routeId).map((id) => { const item = Object.entries(BUYER_PATHS).flatMap(([mode, path]) => path.assemblies.map((assembly) => ({ mode, assembly }))).find((entry) => entry.assembly.id === id); const rule = getSalesRule(id); const buyers = locale === 'de' ? 'Relevante OEM-, Tier-1- und Lieferketten-Einkäufer' : locale === 'fr' ? 'OEM, Tier 1 et acheteurs concernés de la chaîne d’approvisionnement' : locale === 'pt' ? 'OEM, Tier 1 e compradores relevantes da cadeia de suprimentos' : locale === 'es' ? 'OEM, Tier 1 y compradores de la cadena de suministro pertinentes' : locale === 'zh' ? rule.buyers.join('、') : 'Relevant OEM, Tier 1 and supply-chain buyers'; const specifiers = locale === 'de' ? 'Relevante Teams aus Engineering, Qualität und Programmmanagement' : locale === 'fr' ? 'Équipes concernées d’ingénierie, qualité et programmes' : locale === 'pt' ? 'Equipes relevantes de engenharia, qualidade e programas' : locale === 'es' ? 'Ingeniería, calidad y equipos de programas pertinentes' : locale === 'zh' ? rule.specifiers.join('、') : 'Relevant engineering, quality and program teams'; return item && <article key={id}><h3>{pathLabel(item.assembly, locale)}</h3><p><strong>{t(locale, 'sales.directCustomers')}:</strong> {buyers}</p><p><strong>{t(locale, 'sales.technicalSpecifiers')}:</strong> {specifiers}</p></article>; })}</section>)}</div></section>}
    </main>
  );
}
