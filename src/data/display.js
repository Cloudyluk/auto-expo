...
export function localizeLocation(value = '', locale) {
  ...
  return display.replace(/[市省]/g, '').replace(/会展中心/g, locale === 'es' ? 'Centro de Exposiciones' : locale === 'pt' ? 'Centro de Exposições' : locale === 'fr' ? 'Centre des expositions' : 'Exhibition Center').replace(/[\u3400-\u9fff]+/g, locale === 'es' ? 'recinto ferial' : locale === 'pt' ? 'centro de exposições' : locale === 'fr' ? 'site d’exposition' : 'exhibition venue');
}

export function localizeDate(value = '', locale) {
  if (locale === 'zh') return value;
  if (!/\d+月/.test(value)) return value.replace(/待定|待确认/g, locale === 'es' ? 'Por confirmar' : locale === 'pt' ? 'A confirmar' : locale === 'fr' ? 'À confirmer' : 'To be confirmed').replace(/[\u3400-\u9fff]+/g, locale === 'es' ? 'por confirmar' : locale === 'pt' ? 'a confirmar' : locale === 'fr' ? 'à confirmer' : 'to be confirmed');
  ...
  return dated.replace(/(\d{1,2})月（春季）/g, (_, month) => `${months[Number(month) - 1]} (${locale === 'es' ? 'primavera' : locale === 'pt' ? 'primavera' : locale === 'fr' ? 'printemps' : 'spring'})`).replace(/(\d{1,2})月（秋季）/g, (_, month) => `${months[Number(month) - 1]} (${locale === 'es' ? 'otoño' : locale === 'pt' ? 'outono' : locale === 'fr' ? 'automne' : 'autumn'})`).replace(/(\d{1,2})月/g, (_, month) => months[Number(month) - 1]).replace(/待定|待确认|具体/g, locale === 'es' ? 'Por confirmar' : locale === 'pt' ? 'A confirmar' : locale === 'fr' ? 'À confirmer' : 'To be confirmed').replace(/[\u3400-\u9fff]+/g, locale === 'es' ? 'por confirmar' : locale === 'pt' ? 'a confirmar' : locale === 'fr' ? 'à confirmer' : 'to be confirmed');
}
