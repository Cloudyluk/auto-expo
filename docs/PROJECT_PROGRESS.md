...
| P0 数据底座 | 100% | 0% | 数据拆分、字段覆盖、重复合并、市场冲突和严格审计均已完成；地址核验队列单独维护 |
| 官网链接覆盖 | 100% | 0% | 569 条主记录全部已有来源或官方链接 |
| 官网补充批次 | 12 批 / 12 批 | 已完成 | 已新增 392 条规则；每批均独立检查和提交，最后一批补齐剩余 32 条 |
| 重点字段补齐 | 569 / 569 条 | 0 条 | 结构化字段已全覆盖；186 条地址保留“待核验”，进入来源复核维护 |
| 供应链决策层 | 100% | 0% | 优先排序、买卖方建议、详情、收藏、CSV/ICS、冲突和出行提示已实现 |
...
gantt
    ...
    数据 QA 与发布前检查              :done, qa, after split, 3d
    section 已完成：决策工具
    采购/外贸/技术/渠道优先筛选       :done, filters, after qa, 4d
    透明评分与参观决策建议           :done, score, after filters, 5d
    展会详情、重点展商与买家面板      :done, detail, after score, 6d
    收藏、年度清单与 CSV/ICS 导出     :done, plan, after detail, 5d
```
...
- 重点字段当前覆盖：地址字段 569 条、适合人群 569 条、官方来源 569 条；其中 383 条地址已确认，186 条地址待场馆核验。
...
- 为 186 条“待核验”地址补充主办方场馆确认、展商名录和报名入口。
